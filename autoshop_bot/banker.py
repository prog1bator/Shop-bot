...
import banker_database
import banker_keyboard
from banker_config import add_merchant, deposit, user_status_pay, add_message, delete_merchant
...
def start_command(message):
    try:
        ...
        bot.send_message(chat_id,
                         f'💁🏻‍♀️ Правила нашего магазина:\n\n• Осуществление возврата денежных средств за продукцию '
                         f'производится только на внутри лицевой счёт магазина\n• Администрация оставляет за собой '
                         f'право на изменение установленных правил без оповещения пользователей'
                         + '\n• Администрация вправе ограничить доступ определённому лицу без объяснения причины\n• '
                           'Администрация вправе обнулить внутри лицевой счёт пользователя при подозрении в '
                           'мошенничестве\n• Отсутствие видеозаписи - основание для отказа в гарантийном обслуживаниии'
                         + '\n• Гарантия на аккаунты GFN составляет 1 день\n\n💁🏻‍♀️ Вы подтверждаете, '
                           'что *ознакомились и согласны с условиями и правилами* нашего магазина?',
...
def start_command(message):
    try:
        ...
        if chat_id in admin:
            bot.send_message(chat_id, 'Вы вошли в *админ* меню', parse_mode="Markdown",
                             reply_markup=banker_keyboard.admin_keyboard())
    ...
    try:
        ...
        if message.text == '💁🏻‍♀️ Мой профиль':
            ...
            bot.send_message(chat_id,
                             ...
                             f'тебя *{bill}* покупок на сумму *{purchase}* ₽',
                             parse_mode="Markdown", reply_markup=inline_keyboard)
            ...
            if len(rows) == 0:
                ...
                bot.send_message(chat_id, '🚀 Выберите *категорию* товара', parse_mode="Markdown",
            ...
            message = bot.send_message(chat_id,
                                       '💁🏻‍♀️ *Добавление* товара\n*Форма добавления:* '
                                       'категория-наименование-описание-цена-данные',
                                       parse_mode="Markdown")
            ...
            bot.send_message(chat_id, 'Вы вернулись в *основное* меню', parse_mode="Markdown",
                             reply_markup=banker_keyboard.main_keyboard())
        elif message.text == 'Правила':
            bot.send_message(chat_id,
                             f'💁🏻‍♀️ Правила нашего магазина:\n\n• Осуществление возврата денежных средств за '
                             f'продукцию производится только на внутри лицевой счёт магазина\n• Администрация '
                             f'оставляет за собой право на изменение установленных правил без оповещения пользователей'
                             + '\n• Администрация вправе ограничить доступ определённому лицу без объяснения '
                               'причины\n• Администрация вправе обнулить внутри лицевой счёт пользователя при '
                               'подозрении в мошенничестве\n• Отсутствие видеозаписи - основание для отказа в '
                               'гарантийном обслуживаниии'
                             + '\n• Гарантия на аккаунты GFN составляет 1 день\n\n💁🏻‍♀️ Работая с нами, '
                               '*Вы* автоматически *принимаете* правила магазина',
                             parse_mode="Markdown")
    except Exception as e:
        print(e)
    ...
    try:
        ...
        if call.data == 'RULES':
            ...
            bot.send_message(chat_id,
                             ...
                             f'выбор товаров и услуг. *Удачных* покупок!',
                             parse_mode="Markdown",
                             reply_markup=banker_keyboard.main_keyboard())
            g('0', phone, token)
            ...
            if len(rows) == 0:
                bot.send_message(chat_id, f'😔 К сожалению, товары с категорией {regex[1]} *закончились*',
                                 parse_mode="Markdown")
            else:
                inline_keyboard = types.InlineKeyboardMarkup(row_width=2)
                ...
                bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                      text='🚀 Выберите *товар* который хотите приобрести', parse_mode="Markdown",
                                      reply_markup=inline_keyboard)
